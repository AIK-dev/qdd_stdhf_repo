...
!   static 1ph excitation only for vanishing phase angle
    IF (ABS(phangle) + ABS(phphase) == 0D0 .AND. &
        nhstate > 0 .AND. npstate > 0) CALL phstate_static()
#endif
...
! the static iteration starts here
  ...
  DO iter1 = 1, ismax
    ...
    IF (ifsicpsav > 6) THEN ! switch safe pre-iterations for SIC
      IF (iter1 <= itersicp6) THEN   ! 2*istinf) THEN
        ifsicp = 2
      ELSE
        ifsicp = ifsicpsav
      END IF
...
#else
    STOP ' LOCALIZER (switch IFLOCALIZ) should not be invoked in parallele code'
#endif
  END IF
...
#if(mpi)
  LOGICAL :: tocc = .FALSE., tcpu = .FALSE. ! no reoccupation in parallel
#endif
          ...
          IF (iactsp == ispin(nbe)) THEN
            DO nbc = 1, nbe
              IF (iactsp == ispin(nbc)) THEN
                ntridig(iactsp) = 1 + ntridig(iactsp)
                hmatr(ntridig(iactsp), iactsp) = &
                  (hmatrix(nbc, nbe) + hmatrix(nbe, nbc))/2D0
              END IF
            END DO
          END IF
        END DO
...
! WRITE(*,*) ' SSTEP with IFSICP=',ifsicp
  ...
  thamdiag = tmodbuf(iter, ifhamdiag)
...
! optionally compute expectation value of potential energy
    ...
    IF (tmodbuf(iter,istinf)) epotsp(nbe) = wfovlp(q0(:, nbe), q1) + amoy(nbe)
...
! occupied space.
    ...
    IF(istinf > 0) THEN; IF (MOD(iter, istinf) == 0 .AND. ifsicp /= 6) THEN
...
!WRITE(*,*) 'before REOCC:',tocc,nclusrt,nstate*nph,iter,istinf
  IF (tocc .AND. nelect < nstate*nph .AND. iter > 10*istinf) CALL reocc()
...
!WRITE(6,*) ' rate,max=',ncount_rate,ncount_max
  ...
  RETURN
END SUBROUTINE sstep
! finite differences switch finished
#endif
...
!  USE util, ONLY: omega_mieplasmon
  REAL(8), EXTERNAL :: omega_mieplasmon
#endif
