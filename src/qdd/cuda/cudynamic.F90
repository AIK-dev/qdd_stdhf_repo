SUBROUTINE dyn_propag(psi, rho, aloc)
    ...
    use cudadyn
    USE kinetic
    USE util, ONLY: timer, safeopen, testcurrent, rhointxy, rhointyz, rhointxz
    USE RTA_module
    ...
    call init_device_psi()

    call cucalc_ekin()
    ekinsp(:nstate) = ekin_d(:nstate)
    do nb=1,nstate
      CALL calc_ekin(psi(:, nb), ekincpu(nb))
      write(*,'(3F9.6)') ekincpu(nb),ekinsp(nb),abs(ekincpu(nb)-ekinsp(nb))
    enddo
    STOP "TEST cucalc_ekin"
    !allocate(psi_d(kdfull2,nstate))
    !allocate(rho_d(2*kdfull2))
    !allocate(aloc_d(2*kdfull2))
    
    psi_d = psi(:,:nstate)
    occup_d = occup(:nstate)

    !call cudyn_mfield(psi_d)
    ...
    DO it = irest, itmax ! time-loop
        ...
        IF (it > irest) THEN
            ...
            ! protocol of densities
                    ...
                    IF (jdensity1d > 0 .AND. MOD(it, jdensity1d) == 0) THEN
                      CALL rhointxy(rho, it)
                      CALL rhointxz(rho, it)
                      CALL rhointyz(rho, it)
                    END IF
                    IF (jstinf > 0 .AND. MOD(it, jstinf) == 0) CALL testcurrent(psi, rho, it)
...
SUBROUTINE init_dynwf(psi)
  ...
  ! SIC
    ...
    IF (ttimestop) THEN
      CALL system_clock(it5)
      WRITE (6, '(a,10(1pg11.3))') 'systime2: rho,coul,KS,SIC=', &
  ...
  !    END IF
    ...
    IF (myn == 0 .AND. jenergy > 0 .AND. MOD(it, jenergy) == 0. &
        .AND. lprint) THEN
      DO iss = 2, 1, -1
  ...
  ! final composition and PRINT
    ...
    IF (myn == 0 .AND. jenergy > 0 .AND. MOD(it, jenergy) == 0 .AND. lprint) THEN
    ...
    IF (myn == 0) THEN
      ...
      IF (jinfo > 0 .AND. MOD(it, jinfo) == 0 .and. lprint) THEN
  ...
  ! it = time step in calling routine
      ...
      INTEGER :: i
  !---------------------------------------------------------------------
      IF (jdensitydiff /= 0 .AND. MOD(it, jdensitydiff) == 0) THEN
  ...
  !----------------------------------------
      IF (jdensitydiff2d /= 0 .AND. MOD(it, jdensitydiff2d) == 0) THEN
  ...
  !----------------------------------------
      IF (jdensity2d /= 0 .AND. MOD(it, jdensity2d) == 0) THEN
  ...
  !#else
      USE util, ONLY: gettemperature, getcm, safeopen, getclustergeometry
  !#endif
  ...
  ! ?? not clear what that is doing
      IF (jpos > 0 .AND. MOD(it, jpos) == 0 .AND. nelect == 0) THEN
        sumnc = 0D0
        DO i = 1, nc
  ...
  !----------------------------------------------------------------
      ...
      IF (jstateoverlap > 0 .AND. MOD(it, jstateoverlap) == 0) THEN
        CALL stateoverl(psi, psisavex)
      END IF
      ...
      IF ((jstinf > 0 .AND. MOD(it, jstinf) == 0) &
          ...
          .OR. (jdiporb > 0 .AND. MOD(it, jdiporb) == 0)) THEN
            if(it>0) then
              write(*,*) "send gpu data"
              psi(:,:nstate) = psi_d(:,:)
              occup(:nstate) = occup_d(:)
            end if
            call dyn_mfield(rho,aloc,psi,0,0)
  #if(mpi)
  ...
  ! escaped electrons analyzed orbital by orbital
      ...
      IF (jnorms > 0 .AND. MOD(it, jnorms) == 0) THEN
  #if(!mpi)
        CALL safeopen(806, it, jnorms, 'pescOrb')
      ...
      IF (myn == 0 .AND. it > 0 .AND. jangabso > 0 .AND. &
          MOD(it, jangabso) == 0) CALL angular_distribution()
