...
#if(extended)
  NAMELIST /extensions/ ipseudo,  tinterpol, &
    dinmargin, ntref, izforcecorr, &
    isitmax, nhstate, npstate, &
    nhstate2, npstate2, &
    idenspl, i3dz, i3dx, i3dstate, jescmask, jescmaskorb, &
    iscatterelectron, jattach, &
    phangle, phphase, &
    phangle2, phphase2, &
    scatterelectronenergy, &
    ...
    eXUV0, omegaXUV, deltatXUV, deltaAPT, fwhmAPT, ttrain, &
    eXUVx, eXUVy, eXUVz, nAPT,&
    jstochasticint,jjumpspwidth,jjumpaverwidth,jjumpVtrans,iseed,tallowspinexchange
#endif
...
! check consistency of input options
...
#if(mpi)
  IF (ifhamdiag > 0) STOP ' step with H diagonalization ONLY on serial code'
#endif
...
#if(extended)
  IF (ifsicp == 8 .OR. ifsicp == 9) directenergy = .TRUE.
#endif
  ...
  IF (ABS(phangle) > small .AND. .NOT. tstat) &
    STOP ' ph rotation ONLY with ISTAT = .TRUE.'
#endif
  ...
  IF (nion2 == 2) THEN
#if(extended)
    IF (ipseudo) STOP ' IPSEUDO=.FALSE. needed for EXTERNAL psp (NION2=2)'
#endif
...
#if(raregas)
  WRITE (iu, '(a,4i6)') ' nelect,nion,nrare,nstate=', nelect, nion, nrare, nstate
#else
...
!we have a new element
        ...
        IF (naml(3:3) .eq. '1') THEN ! new element is a semi-core
          isaved = icountt - 1 ! semi-core not counted as en element: remember previous element
          icountt = 99
          iskip = 99
...
! go back to READ a new line
      IF (icountt .eq. 99) icountt = isaved ! IF it was a skiped Semi-core, CONTINUE from the previous element
    END DO
...
! Unitialize Coulomb solver, kinetic energy and other grid properties
  ...
  INTEGER :: ix, iy, iz
#if(extended)
  REAL(DP) :: dxfine, dyfine, dzfine, dvolfine  ! should become global
#endif
  REAL(DP) :: x1, y1, z1
!------------------------------------------------------------------
