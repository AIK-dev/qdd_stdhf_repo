...
#if(extended)
  LOGICAL,PARAMETER :: tfermitotal = .TRUE.  ! same temperature for both spins
#endif
...
#if(extended)
    REAL(DP) :: diver0,diver1,diffdiver
    REAL(DP), ALLOCATABLE :: divcurr0(:),divcurr1(:)
    LOGICAL, PARAMETER :: tdiverg = .TRUE. ! switch to divergence of current
#endif
...
#if(extended)
    IF(tdiverg) THEN
      ALLOCATE(divcurr0(kdfull2),divcurr1(kdfull2))
      CALL divergence_vector(curr0,divcurr0)
      diver0 = dvol*SUM(ABS(divcurr0)**2)
    ELSE
      diver0 = 0D0
      diver1 = 0D0
      diffdiver = 0D0
...
#if(extended)
    ELSE
      CALL fermi_perspin(amoy, EspPerSpinRef(1), occup, 1, &
                         rtatempact, rtaferminit, temp(1), mut) 
      CALL fermi_perspin(amoy, EspPerSpinRef(2), occup, 2, &
                         rtatempact, rtaferminit, temp(2), mut) 
    END IF
#endif
...
#if(extended)
        WRITE (*, '(a,i4,20f14.7)') 'iter,EspPerSpinTarget-EspPerSpinRef', &
            j,EspPerSpinTarget(1) - EspPerSpinRef(1), &
...
#if(extended)
        ELSE
          EspPerSpinTarget = EspPerSpinTarget - &
                  (EspPerSpinAchieved - EspPerSpinRef)/2.0d0
          CALL fermi_perspin(EspPerMod, EspPerSpinTarget(1), occup, 1, &
                             max(temp(1) - dT2, rtatempact), &
                             temp(1) + dT2, temp(1), mut)   
          CALL fermi_perspin(EspPerMod, EspPerSpinTarget(2), occup, 2, &
                             max(temp(2) - dT2, rtatempact), &
                             temp(2) + dT2, temp(2), mut)
        END IF
#endif
      ...
      ELSE IF((sumvar2 < rtasumvar2max*1D5 .AND. ERR < errf1) &
        ...
        EspPerSpinTarget = EspPerSpinRef
          CALL fermi_perspin(EspPerMod, EspPerSpinTarget(1), occup, 1, &
                             ...
                             temp(1) + dT1, temp(1), mut)
          CALL fermi_perspin(EspPerMod, EspPerSpinTarget(2), occup, 2, &
                             max(temp(2) - dT1, rtatempact), &
                             temp(2) + dT1, temp(2), mut)
        END IF
#endif
        EspTotAchieved = SUM(EspPerMod(1:nstate)*occup(1:nstate))
...
!old      errj = SUM(ABS(curr1-curr0)**2)/MAX(SUM(ABS(curr0)**2),ma0*1D-4/dvol)
...
#if(extended)
        IF(tdiverg) THEN
          CALL divergence_vector(curr1,divcurr1)
          diver1 = dvol*SUM(ABS(divcurr1)**2)
          diffdiver = dvol*SUM(ABS(divcurr0-divcurr1)**2)
        END IF
        WRITE (177, '(a,i6,20(1pg13.5))') ' DCMF-iteration:', &
          j, ERR, errj, ma0 - ma1, SUM(abs(curr0)**2)*dvol/rho2aver, &
          SUM(abs(curr1)**2)*dvol/rho2aver, &
          EspTotRef - EspTotAchieved, SQRT(sumvar2),&
          entropy(occup),errcoll1/ma0,ABS(ekincoll0-ekincoll1)/ma0,ekincoll0,&
          diver0,diver1,diffdiver
#else
        WRITE (177, '(a,i6,20(1pg13.5))') ' DCMF-iteration:', &
    ...
    CLOSE (1001)
#if(extended)
    IF(tdiverg) DEALLOCATE(divcurr0,divcurr1)
#endif
...
#if(extended)
! anti-commutator for j constraint: the attempts without explicit
!                                   anticommutator can accumulate errors
!                                   ==> still to be tested
    LOGICAL,PARAMETER :: tanticomm=.TRUE. 
#endif
...
#if(extended)
    IF(.NOT.tanticomm) THEN
      CALL xgradient_rspace(Apot(:,1), ALpsi(:,0))
      CALL ygradient_rspace(Apot(:,2), Apsi(:,0))
      ALpsi(:,0) = ALpsi(:,0) + Apsi(:,0)
      CALL zgradient_rspace(Apot(:,3), Apsi(:,0))
      ALpsi(:,0) = ALpsi(:,0) + Apsi(:,0)
      Apot(:,0) = Apot(:,0) -hbarm*ALpsi(:,0)
    END IF
#endif
...
#if(extended)
      COMPLEX(DP), ALLOCATABLE :: gradApsi(:), grad(:)
      COMPLEX(DP), ALLOCATABLE :: jgradpsi(:)
#endif
...
#if(extended)
      ALLOCATE (grad(kdfull2))
      ALLOCATE (jgradpsi(kdfull2))
      IF(tanticomm) ALLOCATE (gradApsi(kdfull2))
#else
...
#if(extended)
      IF(.NOT.tanticomm) THEN
        CALL xgradient_rspace(psi1(1, Nact), grad)
        jgradpsi = 2D0*Apot(:,1)*grad(:) 
        CALL ygradient_rspace(psi1(1, Nact), grad)
        jgradpsi = jgradpsi + 2D0*Apot(:,2)*grad(:) 
        CALL zgradient_rspace(psi1(1, Nact), grad)
        jgradpsi = jgradpsi + 2D0*Apot(:,3)*grad(:) 
      ELSE
#endif
...
!        'Hamilt: j,N,ithr,tests:',j,Nact,ithr2,testdata(1:3)
      ...
      DEALLOCATE (grad)
      DEALLOCATE (jgradpsi)
      IF(tanticomm) DEALLOCATE(gradApsi)
#else
!      DEALLOCATE(gradApsi)
#endif
...
#if(extended)
  SUBROUTINE calc_varsp(hpsi, psi1, sumvar2, sumvar2off, na)
!
! computation of s.p. variance for one s.p. state
    USE params, ONLY: DP, kdfull2, nstate, kstate, ispin, occup, dvol
    USE kinetic
    USE util, ONLY: project, wfovlp, c_project
    IMPLICIT NONE
    COMPLEX(DP), INTENT(IN) :: psi1(kdfull2, kstate)
    COMPLEX(DP), INTENT(IN OUT) :: hpsi(kdfull2)
    REAL(DP), INTENT(IN OUT) :: sumvar2    ! variance from diagonal elements
    REAL(DP), INTENT(IN OUT) :: sumvar2off ! variance with off-diag. subtraction

    REAL(DP) :: evarsp2, evarsp2a
    INTEGER:: na,nb,it1,it2,irate
    COMPLEX(DP) :: ovl   !,norm
    ! hpsi is the set of h*psi in k space
    ! psi1 are the wavefunctions in coordinate space
    LOGICAL, PARAMETER :: ttime = .FALSE.
    LOGICAL, PARAMETER :: tvaroff = .FALSE.
...
!    ALLOCATE(ovlab2(1:kstate))

      CALL fftback(hpsi(:),hpsi(:))
      evarsp2a = REAL(wfovlp(hpsi(:),hpsi(:))) - &
                 ABS(wfovlp(psi1(:,na),hpsi(:)))**2
      sumvar2 = sumvar2 + occup(na)*evarsp2a
      sumvar2off = sumvar2off + occup(na)*evarsp2a
!      WRITE(*,*) 'EVARSP2-R:',na,evarsp2a
      evarsp2a = 0D0
      IF(tvaroff) THEN
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(nb,ovl) SCHEDULE(STATIC) &
!$OMP REDUCTION(+: evarsp2a)
        DO nb = 1,na-1
          IF(ispin(nb)==ispin(na)) THEN
            ovl = dvol*SUM(CONJG(psi1(:,nb)*hpsi(:)))
            evarsp2a = evarsp2a + &
                       occup(na)*occup(nb)*(REAL(ovl,DP)**2 + AIMAG(ovl)**2)
          END IF
        END DO
!$OMP END PARALLEL DO
        sumvar2off = sumvar2off - 2D0*evarsp2a
      END IF

  IF (ttime) THEN
    CALL system_clock(it2)
    WRITE (*, *) 'calc_var CPU:', dble(it2 - it1)/irate
  END IF


  END SUBROUTINE calc_varsp

!_______________________________Fermi_Perspin__________________________________
...
! the total particle number. This is here done for each spin separately.
  SUBROUTINE fermi_perspin(ekmod, eref, occup, ispinact, T0i, T1i, t2, mu)
    USE params, ONLY: DP, kstate, nstate
    IMPLICIT NONE
    REAL(DP), INTENT(IN) :: ekmod(kstate)   ! s.p. energies
    REAL(DP), INTENT(IN) :: Eref            ! reference energy, to be reached
    REAL(DP), INTENT(IN) :: T0i,T1i         ! upper/lower bounds of temperature
    INTEGER, INTENT(IN) :: ispinact         ! spin for which 
    REAL(DP), INTENT(IN OUT) :: occup(kstate) ! final Fermi occupation
    REAL(DP), INTENT(IN OUT) :: mu            ! Fermi energy
...
!    REAL(DP) :: T(2)            !
    ...
    IF(ttest) WRITE (*, *) 'Fermi_Perspin entered:', nspup, nspdw, ispinact, &
                            occref(ispinact), T0i, T1i
    ...
    DO WHILE (abs(T1 - T0) > 1D-10)
      ...
      IF (ispinact .eq. 1) THEN
        CALL occT1(occref(1), ekmod(1:nspup), et, mu, occtot(1), nspup, &
                   T2, occup(1:nspup)) 
      ELSE
        CALL occT1(occref(2), ekmod(nspup + 1:nstate), et, mu, occtot(2), &
                   nspdw, T2, occup(nspup + 1:nstate)) 
      END IF
      IF (et > eref) THEN ! set new bounds in T to embrace solution
        T1 = T2
      ELSE
        T0 = T2
      END IF
      IF(ttest) WRITE (*,'(a,6(1pg13.5))') 'T loop:', &
          t0, t1, et, eref, occref(ispinact), occtot(ispinact)
    END DO
    IF(ttest) WRITE (*, *) 'FERMI_PERSPIN: T,mu,part.nr.=', T2, mu, occtot(ispinact)
  END SUBROUTINE fermi_perspin
#endif
...
!
  ...
  SUBROUTINE divergence_vector(vfield,dvfield)
!
! Divergence of a real vector field 'vfield'. Result is the
! real scalar field 'dvfield'.

    USE params, ONLY: DP, kdfull2
    USE kinetic, ONLY: xgradient_rspace,ygradient_rspace,zgradient_rspace
    IMPLICIT NONE

    REAL(DP), INTENT(IN) :: vfield(kdfull2, 3) 
    REAL(DP), INTENT(OUT) :: dvfield(kdfull2) 

    COMPLEX(DP), ALLOCATABLE :: cvfield(:),accfield(:)

    ALLOCATE(cvfield(kdfull2),accfield(kdfull2))
      
    cvfield = CMPLX(vfield(:,1),0D0,DP)
    CALL xgradient_rspace(cvfield, cvfield)
    accfield = cvfield
    cvfield = CMPLX(vfield(:,2),0D0,DP)
    CALL ygradient_rspace(cvfield, cvfield)
    accfield = accfield + cvfield
    cvfield = CMPLX(vfield(:,3),0D0,DP)
    CALL zgradient_rspace(cvfield, cvfield)
    accfield = accfield + cvfield

    dvfield = REAL(accfield,DP)

    DEALLOCATE(cvfield,accfield)

    RETURN  

  END SUBROUTINE divergence_vector

  SUBROUTINE calc_var(hpsi, psi1, sumvar2, sumvar2off)
!
! computation of s.p. variance all states together
    USE params, ONLY: DP, kdfull2, nstate, kstate, ispin, occup, dvol
    USE kinetic
    USE util, ONLY: project, wfovlp, c_project
    IMPLICIT NONE
    ...
    REAL(DP), INTENT(OUT) :: sumvar2     ! variance from diagonal elements
    REAL(DP), INTENT(OUT) :: sumvar2off  ! variance with off-diag. subtraction
    ...
    LOGICAL, PARAMETER :: tvaroff = .TRUE.
...
!    ALLOCATE(ovlab2(1:kstate))
    ...
    sumvar2off = 0D0
    DO na = 1,nstate
!      norm = wfovlp(psi1(:,na),psi1(:,na))
      CALL fftback(hpsi(:,na),hpsi(:,na))
      evarsp2a = REAL(wfovlp(hpsi(:,na),hpsi(:,na))) - &
                 ABS(wfovlp(psi1(:,na),hpsi(:,na)))**2
      sumvar2 = sumvar2 + occup(na)*evarsp2a
      sumvar2off = sumvar2off + occup(na)*evarsp2a
      evarsp2a = 0D0
      IF(tvaroff) THEN
...
!$OMP REDUCTION(+: evarsp2a)
        DO nb = 1,na-1
          IF(ispin(nb)==ispin(na)) THEN
            ovl = dvol*SUM(CONJG(psi1(:,nb)*hpsi(:,na)))
            evarsp2a = evarsp2a + &
                       occup(na)*occup(nb)*(REAL(ovl,DP)**2 + AIMAG(ovl)**2)
          END IF
        END DO
...
!___________________________________calc_var__________________________________
  SUBROUTINE calc_var_old(hpsi, psi1, sumvar2)
...
!    COMPLEX(DP),ALLOCATABLE :: ovlab(:)
    ! hpsi is the term h psi in k space
    ! psi1 the wavefunctions (in coordinate space)
    sumvar2 = 0D0

!pass hpsi to coordinate space
    DO N = 1, nstate
...
!_______________________________________forceTemp_____________________________
...
!_____________________________ordo_per_spin_________________________________
      SUBROUTINE ordo_per_spin(psi)
        ...
        USE params, ONLY: DP, nspdw, nstate, ispin, occup, kdfull2, kstate
        IMPLICIT NONE
