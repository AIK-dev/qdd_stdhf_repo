...
! Printing informations
  ...
  INTEGER :: izforcecorr = -1, ntref = 0
  LOGICAL :: ipseudo = .FALSE. ! switch on/off subgrids and pseudo-densities
  LOGICAL :: tinterpol = .FALSE.
  !parameters for stochastic
  REAL(DP) ::jjumpspwidth=1.d0
  REAL(DP) ::jjumpaverwidth=1.d0
  REAL(DP) ::jjumpVtrans=1.d0
  INTEGER  ::jstochasticint=0
  REAL(DP) ::energref,spenergref
  INTEGER  ::iseed=0
  LOGICAL  ::tallowspinexchange=.false.
...
#if(mpi)
  INTEGER :: ifhamdiag = 0 ! frequency for Hamiltonian diagonalization
#endif
#if(nompi)
...
#if(extended)
! storage for the caase of 1ph rotation (see 'phangle')
...
! parameters for extended cases and observables
  ...
  INTEGER :: nhstate ! nr. of hole state to be depleted
  INTEGER :: npstate ! nr. of particle state to be filled
  INTEGER :: nhstate2 = 0 ! nr. of 2. hole state to be depleted
  INTEGER :: npstate2 = 0 ! nr. of 2. particle state to be filled
  REAL(DP) :: phangle = 0D0, phphase = 0D0 ! angle and phase of ph rotation
  REAL(DP) :: phangle2 = 0D0, phphase2 = 0D0 ! angle and phase of 2. ph rotation
  REAL(DP) :: trequest = 0D0, timefrac = 0.98D0
  INTEGER :: nproj = 1, nproj_states = 0
  REAL(DP) :: eproj = 0D0, qeproj(kmom), qetarget(kmom)
  REAL(DP) :: vpx = 0D0, vpy = 0D0, vpz = -1D0, taccel = 0D0
...
! below are the parameters for an attopulse train.
! must appear with an IR femtosecond pulse.
  REAL(DP) :: eXUV0 = 0D0, omegaXUV = 0D0, deltatXUV = 0D0, deltaAPT = 0D0, fwhmAPT = 0D0
  REAL(DP) :: ttrain
  REAL(DP) :: eXUVx = 0D0, eXUVy = 0D0, eXUVz = 1D0
  INTEGER :: nAPT
#endif
