...
!COMPLEX(C_DOUBLE_COMPLEX),ALLOCATABLE,PRIVATE :: fftax(:),fftay(:),fftb(:,:),ffta(:,:,:)
  REAL(C_DOUBLE), ALLOCATABLE, PRIVATE :: ffta(:, :, :)
...
! check initialization
    ...
    IF (mini == 0) THEN
      ...
      END IF
      pforw = fftw_plan_r2r_3d(kzmax, kymax, kxmax, ffta, ffta, FFTW_R2HC, FFTW_R2HC, FFTW_R2HC, FFTW_EXHAUSTIVE)
      pback = fftw_plan_r2r_3d(kzmax, kymax, kxmax, ffta, ffta, FFTW_R2HC, FFTW_R2HC, FFTW_R2HC, FFTW_EXHAUSTIVE)
...
!-----fft--------------------------------------------------------------
  ...
  SUBROUTINE fft(psxr, psxi)
    ...
    CALL fftw_execute_r2r(pforw, ffta, ffta)
...
!----------------------------------------------------------------------
    ...
    CALL fftw_execute_r2r(pback, ffta, ffta)
...
!COMPLEX(C_DOUBLE_COMPLEX), INTENT(OUT) :: ffta(nbx2,nby2,nbz2)
    REAL(C_DOUBLE), INTENT(OUT) :: ffta(nbx2, nby2, nbz2)
...
! ffta(MOD(i1+nnx2,nbx2)+1,MOD(i2+nny2,nby2)+1,MOD(i3+nnz2,nbz2)+1)=CMPLX(psxr(ind),0D0,DP)
          ffta(MOD(i1 + nnx2, nbx2) + 1, MOD(i2 + nny2, nby2) + 1, MOD(i3 + nnz2, nbz2) + 1) = psxr(ind)
        END DO
      END DO
    END DO
...
!COMPLEX(C_DOUBLE_COMPLEX), INTENT(IN) :: ffta(nbx2,nby2,nbz2)
    REAL(C_DOUBLE), INTENT(IN) :: ffta(nbx2, nby2, nbz2)
    ...
    DO i3 = 1, nbz2
      DO i2 = 1, nby2
        DO i1 = 1, nbx2
          ind = (i3 - 1)*nxyfn + (i2 - 1)*nyfn + i1
          psxr(ind) = ffta(MOD(i1 + nnx2, nbx2) + 1, MOD(i2 + nny2, nby2) + 1, MOD(i3 + nnz2, nbz2) + 1)
