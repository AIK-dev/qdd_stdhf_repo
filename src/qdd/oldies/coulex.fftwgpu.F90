...
! ikm,nxk,nxklo,nxkhi,nksp,nkxyz,ecut
  COMPLEX(C_DOUBLE_COMPLEX), PRIVATE, POINTER :: fftax(:), fftay(:), fftb(:, :)
...
! check initialization
    ...
    IF (mxini == 0) THEN
      CALL cuda_plan_1d(px, kxmax, batch)
      mxini = kxmax
      WRITE (7, '(a)') ' x-fft initialized '
    ELSE IF (mxini /= kxmax) THEN
      STOP ' nx2 IN four3d not as initialized!'
    END IF
    IF (myini == 0) THEN
      CALL cuda_plan_1d(py, kymax, batch)
      myini = kymax
      WRITE (7, '(a)') ' y-fft initialized '
    ELSE IF (myini /= kymax) THEN
      STOP ' ny2 IN four3d not as initialized!'
    END IF
    IF (mzini == 0) THEN
      CALL cuda_plan_1d(pz, kzmax, batch)
      mzini = kzmax
      WRITE (7, '(a)') ' z-fft initialized '
    ELSE IF (mzini /= kzmax) THEN
...
! execution of the fourier-transformation
        CALL run_fft_for(px, fftax, fftax, kxmax)
...
! negative space
        ...
        END DO
! execution of the fourier-transformation
        CALL run_fft_for(py, fftay, fftay, kymax)
...
!test DO i1=1,kfftx
      DO i1 = nx, nxi ! 1,nx1
        CALL run_fft_for(pz, fftb(1, i1), fftb(1, i1), kfftz)
      END DO
      DO i3 = 1, kfftz
        i3m = MOD(i3 + nzh, kfftz) + 1
!test DO i1=1,kfftx
...
!----------------------------------------------------------------------
    ...
    DO i2 = 1, kffty
      ...
      END DO
      DO i1 = nx, nxi ! 1,nx1
        CALL run_fft_back(pz, fftb(1, i1), fftb(1, i1), kzmax)
      END DO
...
! execution
        CALL run_fft_back(py, fftay, fftay, kymax)
...
! execution
        CALL run_fft_back(px, fftax, fftax, kfftx)
...
! ******************************
    ...
    CALL c_f_pointer(c_p_fftax, fftax, [kxmax])

    res = cudaMallocHost(c_p_fftay, nby2*sizeof(size_cmplx))
    CALL c_f_pointer(c_p_fftay, fftay, [kymax])

    res = cudaMallocHost(c_p_fftb, nbz2*nbx2*sizeof(size_cmplx))
    CALL c_f_pointer(c_p_fftb, fftb, [kzmax, kxmax])
