...
implicit none
	...
	REAL(DP)         :: rhosto(2*kdfull2)
	!--------------------------
	INTEGER       :: i,nb,ordering(kstate)
	REAL(DP)      :: sumEout,sumeoutref,spenerg(kstate),energref1, spenergref1
  ...
  do i=1,nstate; 
  	...
  	spenergref=spenergref+(amoy(i)+eout(i,i))*occup(i)! in params
	sumeoutref=sumeoutref+eout(i,i)*occup(i)
  enddo
  write(*,*)'enegref, spenergref', energref,spenergref

  write(*,*) 'before hdiag'
  call hdiag_part_hole(psi,rho,aloc,MoP,Eout,spenerg)
  call jump_prob_open(psi,rho,aloc,spenerg,it,eout,MoP)
!put everything back in params
	call calcrho(rho,psi)
	call dyn_mfield(rho, aloc, psi, dt1, it)
	call info(psi, rho, aloc, it, .false.)
	energref1=energy
	...
	sumeout=0
	do i=1,nstate; 
  		energref1=energref1+eout(i,i)*occup(i);
  		spenergref1=spenergref1+(amoy(i)+eout(i,i))*occup(i)
		sumeout=sumeout+eout(i,i)*occup(i)
	enddo
  write(*,'(a,4f10.7)')'energref, spenergref,energref1, spenergref1', energref,spenergref,energref1, spenergref1
  write(*,'(a,4f10.7)')'sumeoutref, sumeout', sumeoutref, sumeout
  write(*,'(a,4f10.7)')'drho', sum(abs(rho-rhosto))


end subroutine stochastic
!________________________________jump_prob_open_____________________________________
SUBROUTINE jump_prob_open(psi,rho,aloc,spenerg,it,eout,MoP)
	...
	COMPLEX(DP), INTENT(IN)         :: MoP(kstate,kstate)
	REAL(DP)   , INTENT(IN OUT)         :: spenerg(kstate)
    INTEGER    , INTENT(IN)             :: it
    COMPLEX(DP), INTENT(IN OUT)             :: eout(kstate,kstate)
	REAL(DP) , INTENT(IN OUT)           :: aloc(2*kdfull2)
	REAL(DP) , INTENT(IN OUT)           :: rho(2*kdfull2)
    ...
    COMPLEX(DP)         :: trprb,eoutdiag(kstate,kstate)
    INTEGER*4 ::getpid
	real(dp):: energrefd,spenergrefd
    !--------------------------
    !Eout is in original basis 
	!check energies
	call calcrho(rho,psi)
	call dyn_mfield(rho, aloc, psi, dt1, it)
	call info(psi, rho, aloc, it, .false.)
	energrefd=energy;
  	spenergrefd=0.d0
  do i=1,nstate; 
  	energrefd=energrefd+eout(i,i)*occup(i)
	spenerg(i)=amoy(i)+eout(i,i)
	spenergrefd=spenergrefd+spenerg(i)*occup(i)! in params
  enddo
  write(*,'(a, 6f12.8)') 'energref, energrefd, spenergref, spenergrefd',&
						  energref, energrefd, spenergref, spenergrefd
	!ispin, occup, spenerg are in basis diag;
	if(iseed.eq.0) iseed=-getpid()!ran2 renvoie iidum=1 sinon le iseed lu
    WRITE(*,*) 'iseed : ',iseed
...
!_____________eout in diag basis
	 	eoutdiag=matmul(transpose(conjg(MoP)),Eout)!
     	eoutdiag=matmul(eoutdiag,MoP)!
...
ih1loop:DO ih1=1,nelect
             DO ih2=ih1+1,nelect
                DO ip1=nelect+1,nstate
                    ...
                    IF ((ip1.ne.ip2).and.&
                        ((ispin(ip1).eq.ispin(ih1)).and.(ispin(ip2).eq.ispin(ih2))).or.&
						(tallowspinexchange.and.&
						(ispin(ip1).eq.ispin(ih2)).and.(ispin(ip2).eq.ispin(ih1)))) THEN
                    write(*,*) ih1,ih2,ip1,ip2
                    delta_spe = spenerg(ih1) + spenerg(ih2) &
                                - spenerg(ip1) - spenerg(ip2)! & spenerg buit on total energy 
!                                +REAl(eoutdiag(ih1,ih1),8)+REAL(eoutdiag(ih2,ih2),8) &! do not add the eout again. 
!                               -REAL(eoutdiag(ip1,ip1),8)-REAL(eoutdiag(ip2,ip2),8)
                    ncount=ncount+1
                    ...
                    if (abs(delta_spe) < jjumpspwidth) then 
						...
						call dyn_mfield(rho, aloc, psi, dt1, it)
						call info(psi, rho, aloc, it, .false.)
						eouttot=0.d0;do i =1,nstate; eouttot=eouttot+occup(i)*eoutdiag(i,i);enddo
	...
	Eout=Eoutdiag! we will remain in the basis diag so keep eout in basis diag
    

END SUBROUTINE jump_prob_open
!_________________________________hdiag_part_hole___________________________________
SUBROUTINE hdiag_part_hole(psi,rho,aloc,actMoP,Eout,spenerg)!mv
	USE params 
	...
	!--------------------------
	COMPLEX(DP),allocatable   :: hmatr(:,:),vect(:,:),hmatrst(:,:)
	COMPLEX(DP),allocatable   :: eoutact(:,:),psiaux(:)
	 REAL(DP),allocatable      :: eigen(:)
	 real(dp)::spenergtot0,spenergtot
	 !--------------------------
	INTEGER       :: nb , st,i,j,ishift
	INTEGER       :: nelecup,nelecdown
	allocate(hmatr(kstate,kstate),hmatrst(kstate,kstate),vect(kstate,kstate),eoutact(kstate,kstate),eigen(kstate))
	write(*,*) 'before sortmop'
	call sort_MoP(psi,actMoP)! psi, occ and ispin come back in ordo basis
	 eoutact=matmul(transpose(conjg(actMoP)),Eout)!needed to compute hmatr, in ordo basis
     eoutact=matmul(eoutact,actMoP)!needed to compute hmatr, in ordo basis eout=v* eout v
	 !check energy and energtot before diagonalisation
	call calcrho(rho,psi)
	call dyn_mfield(rho, aloc, psi, dt1, 0)
	call info(psi, rho, aloc, 0, .false.)
	spenergtot0=0.d0;do nb=1,nstate; spenergtot0=spenergtot0+occup(nb)*(amoy(nb)+eoutact(nb,nb));enddo
	
	 !____________initialyze H_ij
	 ...
	 do nb = 1, nstate
	 	psiaux=psi(:,nb)
		ishift = (ispin(nrel2abs(nb)) - 1)*nxyz!aloc is unchanged from dynamic
        CALL hpsi(psiaux, aloc(ishift + 1), nb, 1)!psi,aloc not altered
	 	...
	 	hmatr=hmatr+eoutact! diagonalyse energy tot(in + out)
	 hmatrst=hmatr
	 write(*,*) 'hmatr'
	 ...
	 actMoP=matmul (actMop,vect)!Mop was in ordo basis, now from std to diag
	 !spenerg=eigen!spenerg contains the eout but eigen are not the real sp energies!!!
     ...
     write(*,'(20i3)')ispin
	 !now check 
	! write(*,'(20 f7.4)') abs(matmul( matmul(vect,hmatrst) ,transpose(conjg(vect))))
	 write(*,*)'hmatr 2'
	 write(*,'(20 f7.4)') abs(matmul(matmul(transpose(conjg(vect)),hmatrst) ,vect ))
	call calcrho(rho,psi)
	call dyn_mfield(rho, aloc, psi, dt1, 0)
	call info(psi, rho, aloc, 0, .false.)
	 !____________initialyze H_ij check
	 hmatr(:,:) = cmplx(0.d0,0.d0,8)
	 vect(:,:) = cmplx(0.d0,0.d0,8)
	 allocate(psiaux(kdfull2))!only one column, to avoid a too large auxiliary matrix
	 !__________________________create check H_ij
	 do nb = 1, nstate
	 	psiaux=psi(:,nb)
		ishift = (ispin(nrel2abs(nb)) - 1)*nxyz!aloc is unchanged from dynamic
        CALL hpsi(psiaux, aloc(ishift + 1), nb, 1)!psi,aloc not altered
	 ...
	 deallocate(hmatr,hmatrst,vect,eoutact,eigen)

 end SUBROUTINE hdiag_part_hole	
 

SUBROUTINE sort_MoP(psi,MoP)! psi, spnergy, occup
...
spnerg(:nstate) = spnerg(:nstate) + ekinsp(:nstate)
	...
	allocate(whpsi(kdfull2))
	  DO nb=1,nstate
	    whpsi = psi(:,nb)
		ishift = (ispin(nb) - 1)*nxyz
	    CALL hpsi(whpsi,aloc(ishift+1),nb,0) ! with "0", "nb" is not usefull
